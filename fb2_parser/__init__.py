...
import gettext
from .constants import STRING_TAGS, GENRES, MESSAGES
class ParsingError(Exception):
    def __init__(self, error_message):
        self.error_message=error_message
    def get_error(self):
        return self.error_message
class Element_not_found(ParsingError):
    def __init__(self, element_name):
        super().__init__(('element_not_found', {'element_name': element_name}))
class Attribute_not_found(ParsingError):
    def __init__(self, attribute_name):
        super().__init__(('attribute_not_found', {'attribute_name': attribute_name}))
class FB2Parser:
    ...
    def _parse(self):
        ...
        if not fb:
            raise Element_not_found('FictionBook')
    ...
    def parse_fictionbook(self, fb):
        if not fb.description:
            raise Element_not_found('description')
        if not fb.body:
            raise Element_not_found('body')
    ...
    def parse_description(self, description):
        if not description.find('title-info'):
            raise Element_not_found('title-info')
        #if not description.find('document-info'):
            #raise Element_not_found('document-info')
    ...
    def parse_body(self, body):
        #if not body.section:
            #raise Element_not_found('section')
        result=''
        structure=[]
        html_structure = []
        name=body.get('name')
    ...
    def parse_some_title_info(self, title_info):
            ...
            #raise Element_not_found('genre')
        #if not title_info.author:
            #raise Element_not_found('author')
        #if not title_info.find('book-title'):
            #raise Element_not_found('book-title')
    ...
    def parse_title_as_text(self, title):
        ...
        for c in title.children:
            ...
            if c.name=='empty-line':
                ...
                html_structure += t[1]
            if t:
                raw_html += f'<{t[1][0][0]}>{t[1][0][1]}</{t[1][0][0]}>' if t[1][0][0]!=None else t[1][0][1]
